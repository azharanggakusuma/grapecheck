...
const AccordionSection = ({ title, content, colors }: any) => {
  ...
  const handlePress = () => {
    ...
    Animated.timing(anim, {
      ...
      duration: 250,
      useNativeDriver: false, // necessary for layout animation
    }).start();
  };
...
export default function CheckScreen() {
  ...
  const pickImage = async (useCamera: boolean) => {
    ...
    } catch (e) { Alert.alert("Error", "Gagal membuka gambar."); }
  };
  ...
  const classifyImage = async (uri: string) => {
    ...
    try {
      const response = await fetch(CLASSIFY_URL, { method: "POST", body: formData, headers: { "Content-Type": "multipart/form-data" } });
...
const styles = StyleSheet.create({
  ...
  imageResultContainer: {
    ...
    shadowRadius: 15,
    elevation: 10,
    overflow: 'hidden', // to ensure blur is contained
  },
